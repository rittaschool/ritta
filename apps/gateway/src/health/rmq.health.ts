/*
    This file is from the @nestjs/terminus project originally licensed under MIT.
*/
import { HealthCheckError } from '@nestjs/terminus/dist/health-check/health-check.error';
import { Injectable, Scope } from '@nestjs/common';
import * as NestJSMicroservices from '@nestjs/microservices';
import { TimeoutError } from '@nestjs/terminus/dist/errors';
import {
  checkPackages,
  promiseTimeout,
...
export class MicroserviceHealthIndicator extends HealthIndicator {
  ...
  private async pingMicroservice<MicroserviceClientOptions>(
    ...
    const checkConnection = async () => {
      ...
      await client
        .send(IEventType.STATUS, {})
        .pipe(timeout(50)) // timeout is short because terminus runs them after each other
        .toPromise()
        .catch(() => {
          throw new Error('Microservice did not respond');
        });
      await client.close();
    };
