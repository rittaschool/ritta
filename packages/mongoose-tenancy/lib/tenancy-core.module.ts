import { BadRequestException, DynamicModule, Global, Inject, Injectable, Module, OnApplicationShutdown, Provider, Scope } from '@nestjs/common';
...
export class TenancyCoreModule implements OnApplicationShutdown {
    ...
    private static async getTenant(
        ...
        // TODO: Use the Ritta Backend Framework to resolve the tenant id
        if (getTenant && typeof getTenant === 'function') {
            return await getTenant(req);
        } else if (isTenantFromSubdomain) {
    ...
    private static async getConnection(
        ...
        modelDefMap.forEach(async (definition: any) => {
            ...
            if (moduleOptions.forceCreateCollections) {
                // For transactional support the Models/Collections has exist in the
                // tenant database, otherwise it will throw error
                await (modelCreated as any).createCollection();
            }
        });
    ...
    private static createTenantContextProvider(): Provider {
        return {
            ...
            useFactory: (
                ...
                moduleOptions: TenancyModuleOptions,
                adapterHost: HttpAdapterHost,
            ): Promise<string> => this.getTenant(req, moduleOptions, adapterHost),
            inject: [
                REQUEST,
                TENANT_MODULE_OPTIONS,
